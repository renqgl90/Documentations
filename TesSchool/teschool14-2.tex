...
\maketitle\abstracts{
The work presented consists in a preliminary study for the measurement of the branching ratio of the doubly charmed decay ${ B }^{ 0 }\rightarrow { D }^{ 0 }\overline { { D }^{ 0 } } { K }^{ *0 } + C.C.$ (never measured and observed so far) using the ${ B }^{ + }\rightarrow { D }^{ 0 }\overline { { D }^{ 0 } } { K }^{ + } +C.C.$ as reference channle. The $D$ mesons are reconstructed through $\Dzch +c.c$.
...
$B^{0}\rightarrow D^{0} \overline{D}^{0} K^{\ast 0}  +  c.c.$ are generally described by the \textit{CKM} favoured weak transition $\overline{b} \rightarrow \overline{c} W^{+} \rightarrow \overline{c} c \overline{s}$ with additional $u\overline{u}$(or $d\overline{d} $ quarks popping up from \textit{QCD} vacum. The relevant feynman diagrams for such decay is shown in Fig \ref{quarks}.
\begin{figure}[h!]
\begin{center}
\includegraphics[ width=0.5\textwidth]{figs/Feynman2.png}
\caption{Leading order feynman diagrams for $\Bpch$. On the right the color favoured mode ($u\overline{u}$ from \textit{QCD} vacuum can have 3 different color assignment), on the left the coler suppressed mode (once the spectator quark color is fixed, all the other are fixed). For $\Bpch$ both the modes can occur while for the $\Bzch$ only the suppressed mode can occour.}\label{quarks}
\end{center}
\end{figure}

\footnote{This decay modes have been intruduced due to the so called \textit{Charm counting puzzle}, i.e., theoretical discrepancy between the semileptonic branching ratio of $B$ decays with the number of charmed hadrons produced in $B$  decays.} Hadrons are colourless objects so the decay modes containing $D^{(\ast)}$ as the ones of interest are produced in majority through Internal (color suppressed) and/or external modes (color favoured). The reference channel occour through both of them while the $\Bzch$ only internally.

The two major  studies which can be done through this decays are:
Spectroscopy of $c\overline{s} $ and $c\overline{c}$ states. Resonances coming from  $D^{*}_{s} \rightarrow D^{*} K^{*}$ and  $X->D^{*}D^{*}$ can be studied in 3-body decays through \textit{Dalitz Amplitude Analysis technique}. The physics case is more interesting for the second family.\footnote{ There are several discrepancies between observed states and predicted one and the nature of some of the observed one is not clear and unambiguosly defined}.
...
\caption{Reconstructed $B$ mass spectrum for  the 2011 and 2012 LHCb data in the $B^{\pm}\rightarrow D^{0} \overline{D}^{0} K^{\pm}$ channel. \textit{Data Stripping + Filter} are the pre-selected \textit{B} candidates. 
The \texti{BDTCat} is the \textit{MVA} classifier which permits to separate background-like and signal-like events.
The cut value \textit{-0.15} has been choosen maximising the $\frac{S}{\sqrt{S+B}}$ figure of merit (here $\sim 40$).
The two peaks below the nominal $B^{+}$ mass  consists into reconstruced $D$ mesons from the strong $D^{\ast}\rightarrow D \pi$ decay where the $\pi$ is outside the LHCb acceptance.}\label{Bmass}
\end{center}
\end{figure}

Using the rather clean sample of $B$ candidates a preliminary Dalitz analysis has been done looking to the \textit{2-D} plot build using the $M^{2}_{A+B}$ and $M^{2}_{B+C}$ invariant masses distributions out of a 3-body decay $X\rightarrow ABC$ decay. The obtained plot is shown in Fig \ref{Dalitz}.
\begin{figure}[h!]
\begin{center}
\includegraphics[ width=0.7\textwidth]{figs/Dalitz1.pdf}
\caption{$\Bpch$ Dalitz Plot of $(D^{0}\overline{D}^{0})$ vs $(D^{0}K^{+}+c.c.)$. The accumulation of events on the top-left part are related to $B^{+}\rightarrow \psi(3770)K^{+}\rightarrow D^{0}\overline{D}^{0} K^{+}$ in fact the $\psi(3770)$ has a $J=1$ and the corresponding typical pattern is the one that can be seen (two peaks, one zero). In addition another resonance is present in the central horizontal line and it correspond to a $J=1$ resonance due to $D^{*}_{s1}(2700)\rightarrow D^{0}K^{\pm}$.}\label{Dalitz}
\end{center}
\end{figure}
